...
- incoming group offers arrive through realtime group gossip or group offline catch-up, persist as pending group file rows, and emit the same pending-file renderer event. Group capacity/rate rejection still sends no direct NACK to the sender, but a capacity-full drop emits a local pending-file deferred event so the recipient UI can show the pending-file manager and explain that clearing older offers plus checking missed messages can recover skipped group offers. Group Reject is a local terminal transition only, and group Accept runs the same app-key-authenticated pull protocol as direct offers
